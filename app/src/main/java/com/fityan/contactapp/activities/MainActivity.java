...
public class MainActivity extends AppCompatActivity implements ContactAdapter.OnItemListener {
  ...
  public void onDeleteItem(int position) {
    ...
    AlertDialog dialog = new AlertDialog.Builder(this).setTitle("Delete Contact")
        ...
        .setPositiveButton("Delete Contact", (dialogInterface, i) -> {
          /* Delete contact */
          contactsCollection.delete(contacts.get(position).getId()).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
              /* Refresh the contact list view */
              onRestart();
              Toast.makeText(this, "One contact has been deleted.", Toast.LENGTH_SHORT).show();
            } else {
              Log.e("deleteContact", "Failed to delete contact.", task.getException());
  ...
  private void loadContactsFromDB() {
    ...
    contactsCollection.findAll(user.getUid()).addOnSuccessListener(queryDocumentSnapshots -> {
      for (DocumentSnapshot document : queryDocumentSnapshots.getDocuments()) {
        contacts.add(
            new Contact(document.getId(), document.getString("name"), document.getString("phone"),
                document.getString("email"), document.getString("address"), user.getUid()));
      }
